...
fn main() {
    ...
    let dependencies: Vec<Dependency> = parse_yaml(filename);
...
// return vector
fn parse_yaml(yaml_file: &str) -> Vec<Dependency> {
    ...
    let dependencies_hash_map = contents[0]["attributes"]["system"]["dependencies"].as_vec().unwrap();

    for hash_map in dependencies_hash_map {
